...
class MetroAgi:
    ...
    def heuristik(self, istasyon1: 'Istasyon', istasyon2: 'Istasyon') -> int:
        # Örnek olarak sabit bir değer döndürüyoruz. Gerçek bir uygulamada, coğrafi mesafeye göre hesaplanabilir.
        return abs(int(istasyon1.istasyon_kodu.split('_')[1]) - int(istasyon2.istasyon_kodu.split('_')[1]))
...
if __name__ == "__main__":
    ...
    # Senaryo 1: ODTÜ' den BATIKENT'e
    print("\n1. ODTÜ' den BATIKENT'e:")
    rota = metro.en_az_aktarma_bul("M2_7", "M1_12")
    if rota:
        print("En az aktarmalı rota:", " -> ".join(istasyon.istasyon_adi for istasyon in rota))
    
    sonuc = metro.en_hizli_rota_bul("M2_7", "M1_12")
    if sonuc:
        rota, sure = sonuc
